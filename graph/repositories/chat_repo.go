...
func (repo *ChatRepo) FindAllByUserId(ctx context.Context, userId uuid.UUID) ([]*entity.Chat, error) {
	chats := []*entity.Chat{}
	repo.DB = repo.DB.WithContext(ctx)
	err := repo.BaseRepo.DB.Preload("Messages.Author").Preload(clause.Associations).Find(&chats, "user_id = ?", userId)
	if err.Error != nil {
...
func (repo *ChatRepo) ChatExists(ctx context.Context, userID, targetID uuid.UUID) (*entity.Chat, error) {
	repo.DB = repo.DB.WithContext(ctx)
	chat := &entity.Chat{}
	err := repo.DB.Joins("JOIN chat_participants ON chats.id = chat_participants.chat_id AND chats.user_id = ? AND chat_participants.user_id = ?", userID, targetID).First(chat).Error
	if err != nil {
		return nil, err
	}
